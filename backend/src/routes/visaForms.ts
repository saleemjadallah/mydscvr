 ...
 * Analyze PDF form using Azure Document Intelligence (with Gemini fallback)
 *
 * NOTE: This endpoint expects a PDF file buffer, not base64 images.
 * The frontend should send the original PDF file directly.
 */
router.post('/analyze-pdf', async (req: Request, res: Response) => {
  try {
    ...
    // Use document router which tries Azure first, then falls back to Gemini
